...
def index():
    # 이 코드가 templates/nhl_dashboard.html 파일을 찾아 화면에 띄워줍니다.
    return render_template('nhl_dashboard.html')

# 선수 검색 로직 (나중에 데이터 분석 기능을 추가할 부분)
@app.route('/search', methods=['POST'])
def search():
    player_name = request.form.get('player_name')
    # 여기에 NHL API 분석 로직이 들어갈 예정입니다.
    return jsonify({"message": f"{player_name} 데이터를 분석 중입니다..."})
